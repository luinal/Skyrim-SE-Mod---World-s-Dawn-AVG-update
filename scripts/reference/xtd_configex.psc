...
Function AddLoot(Actor akToCheck)
	...
	Endif
    If (akToCheck.GetItemCount(Gold001) > 0 && PlayerRef.GetActorValue("FavorPointsBonus") > 0.0)
        int BonusGold = (akToCheck.GetItemCount(Gold001) * (PlayerRef.GetActorValue("FavorPointsBonus")/ 100.0)) as int
