...
Function CheckEquipment(Actor akToCheck, Actor p)
	...
	If akToCheck.HaskeywordString("ActorTypeNPC") || (akToCheck.HaskeywordString("ActorTypeCreature") && !akToCheck.HaskeywordString("ActorTypeAnimal"))
		float odds = XTDGV_DropRateMult.GetValue() * XTDUtil.CalculateMyOdds(p, "MagicItemFindMod", (XTDGV_MIF.GetValue()))
		float TemperChance = XTDGV_DropRateMult.GetValue() * XTDUtil.CalculateMyOdds(p, "TemperedItemFindMod", (XTDGV_TIF.GetValue()))
		XTD.CalculateOdds(p)
		if (akToCheck.GetLevel() - p.GetLevel() >= 5) || (Utility.RandomFloat(0,100) <= (Attributes.GetTotalValue(6)/7))
			BetterLoot = true
		endif
		if !(akToCheck.IsDead())
	...
	Endif
    If (akToCheck.GetItemCount(Gold001) > 0 && p.GetActorValue("GoldFindMod") > 0.0)
        int BonusGold = (akToCheck.GetItemCount(Gold001) * (p.GetActorValue("GoldFindMod")/ 100.0)) as int
